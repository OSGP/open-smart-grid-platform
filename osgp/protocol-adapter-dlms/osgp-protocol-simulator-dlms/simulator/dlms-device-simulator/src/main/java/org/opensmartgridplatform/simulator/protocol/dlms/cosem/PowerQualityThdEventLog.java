...
@CosemClass(id = 7)
public class PowerQualityThdEventLog extends ProfileGeneric {
  ...
  /**
   * Only for cosem attribute definition, data remains untouched. Attribute data is gathered from
   * {@link #CAPTURE_OBJECT_DEFINITIONS}
   */
  @CosemAttribute(
      id = 3,
      type = Type.ARRAY,
  ...
  public PowerQualityThdEventLog(final Calendar time) {
    super("0.0.99.98.8.255");
    this.time = time;
  ...
  private static CaptureObjectDefinitionCollection initCaptureObjects() {
    ...
    definitions.add(
        new CaptureObjectDefinition(
            new CaptureObject(1, "0.0.96.11.8.255", (byte) 2, 0), new UInteger16DataProcessor()));

    // Magnitude
    definitions.add(
        new CaptureObjectDefinition(
            new CaptureObject(1, "0.0.96.11.22.255", (byte) 2, 0), new UInteger16DataProcessor()));
