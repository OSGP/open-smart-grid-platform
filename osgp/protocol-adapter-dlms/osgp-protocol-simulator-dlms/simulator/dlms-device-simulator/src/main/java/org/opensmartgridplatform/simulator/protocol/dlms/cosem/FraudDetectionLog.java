 ...
 * Fraud detection log.
 *
 * <p>Simulates data from the fraud detection log.
 */
@CosemClass(id = 7)
public class FraudDetectionLog extends ProfileGeneric {
  ...
  public FraudDetectionLog(final Calendar time) {
    super("0.0.99.98.1.255");
    this.time = time;
  ...
  private static CaptureObjectDefinitionCollection initCaptureObjects() {
    ...
    definitions.add(
        new CaptureObjectDefinition(
            new CaptureObject(1, "0.0.96.11.1.255", (byte) 2, 0), new UInteger8DataProcessor()));
