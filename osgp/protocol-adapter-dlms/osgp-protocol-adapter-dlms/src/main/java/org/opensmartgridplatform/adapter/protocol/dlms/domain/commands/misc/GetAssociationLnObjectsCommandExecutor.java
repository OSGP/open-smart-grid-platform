...
public class GetAssociationLnObjectsCommandExecutor extends AbstractCommandExecutor<Void, AssociationLnListTypeDto> {
    ...
    public AssociationLnListTypeDto execute(final DlmsConnectionManager conn, final DlmsDevice device,
        ...
        final List<GetResult> getResultList = DlmsHelper
    ...
    private List<AssociationLnListElementDto> convertAssociationLnList(final List<DataObject> resultDataValue)
        ...
        for (final DataObject obisCodeMetaData : resultDataValue) {
            ...
            final AssociationLnListElementDto element = new AssociationLnListElementDto(
                    DlmsHelper.readLong(obisCodeMetaDataList.get(CLASS_ID_INDEX), "classId"), version,
                    DlmsHelper.readLogicalName(obisCodeMetaDataList.get(OBIS_CODE_INDEX), "AssociationLN Element"),
    ...
    private AccessRightDto convertAccessRights(final DataObject dataObject) throws ProtocolAdapterException {
        ...
        final List<DataObject> attributeAccessDescriptorObjects = accessRights.get(ACCESS_RIGHTS_ATTRIBUTE_ACCESS_INDEX)
                .getValue();
        final AttributeAccessDescriptorDto attributeAccessDescriptor = this
                .convertAttributeAccessDescriptor(attributeAccessDescriptorObjects);

        final List<DataObject> methodAccessDescriptorObjects = accessRights.get(ACCESS_RIGHTS_METHOD_ACCESS_INDEX)
                .getValue();
        final MethodAccessDescriptorDto methodAccessDescriptor = this
                .convertMethodAccessDescriptor(methodAccessDescriptorObjects);
        ...
        for (final DataObject attributeAccessItemRaw : attributeAccessDescriptor) {
            ...
            attributeAccessItems.add(new AttributeAccessItemDto(
                    DlmsHelper.readLong(attributeAccessItem.get(ACCESS_RIGHTS_ATTRIBUTE_ACCESS_ATTRIBUTE_ID_INDEX), "")
                            .intValue(), AttributeAccessModeTypeDto.values()[DlmsHelper
    ...
    private List<Integer> convertAccessSelectors(final List<DataObject> accessSelectors)
        ...
        for (final DataObject accessSelectorRaw : accessSelectors) {
            convertedAccessSelectors.add(DlmsHelper.readLong(accessSelectorRaw, "").intValue());
        }
        return convertedAccessSelectors;
    }

    private MethodAccessDescriptorDto convertMethodAccessDescriptor(final List<DataObject> methodAccessDescriptor)
            throws ProtocolAdapterException {
        ...
        for (final DataObject methodAccessItemRaw : methodAccessDescriptor) {
            ...
            methodAccessItems.add(new MethodAccessItemDto(
                    DlmsHelper.readLong(methodAccessItem.get(ACCESS_RIGHTS_METHOD_ACCESS_METHOD_ID_INDEX), "")
                            .intValue(), MethodAccessModeTypeDto.values()[DlmsHelper
