 ...
 * <p>
 * The ManufacturerId in its textual form is numerically encoded in a kind of
 * base-32 encoding with a restricted set of values (AAA..ZZZ).<br>
 * Each of the three digits of a manufacturer ID is an upper case letter where
 * 'A' -> 1, 'B' -> 2 and 'Z' -> 26. Since ASCII('A') = 65, the numerical value
 * for each of the digits can be calculated by subtracting 64 from the ASCII
 * value.<br>
 * The integer value is the base-10 value calculated from the base-32
 * representation as described above. This value is the one that is used for the
 * manufacturer_id (attribute 7) of the DLMS M-Bus client (class ID 72).
 * <p>
 * The DLMS User Association has a list of appointed IDs.
 *
 * @see MbusClientAttribute#MANUFACTURER_ID
 * @see <a href="http://www.dlms.com/organization/flagmanufacturesids/">FLAG
 *      Manufacturers ID</a>
 */
public class ManufacturerId {
    ...
    private static final int MIN_ID = 1 * MANUFACTURER_ID_BASE * MANUFACTURER_ID_BASE + 1 * MANUFACTURER_ID_BASE + 1;
    private static final int MAX_ID = 26 * MANUFACTURER_ID_BASE * MANUFACTURER_ID_BASE + 26 * MANUFACTURER_ID_BASE + 26;
    ...
    public static ManufacturerId fromDataObject(final DataObject dataObject) {
        ...
        if (dataObject == null || dataObject.isNull()) {
            ...
            throw new IllegalArgumentException("dataObject type must be long-unsigned: " + dataObject.getType());
        } else {
    ...
    /**
     * Calculates the integer value of the DLMS M-Bus client setup
     * manufacturer_id from the given manufacturer identification, according to
     * EN 62056-21.
     * <p>
     ...
     * </table>
     * <p>
     * ASCII('A') = 65, ASCII('B') = 66, ..., ASCII('Z') = 90
     *
     * @param identification
     *            manufacturer identification in three capital letters
    ...
    public String toString() {
        return String.format("ManufacturerId[%s(%d)]", this.identification, calculateId(this.identification));
    }
