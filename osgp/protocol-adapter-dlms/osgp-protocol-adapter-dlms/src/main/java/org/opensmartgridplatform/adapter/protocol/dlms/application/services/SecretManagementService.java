...
public class SecretManagementService {
  ...
  public Map<SecurityKeyType, byte[]> getKeys(
    ...
    final GetSecretsResponse response =
        this.secretManagementClient.getSecretsRequest(correlationUid, request);
  ...
  public byte[] getNewKey(
    ...
    return this.getNewKeys(correlationUid, deviceIdentification, Arrays.asList(keyType))
        .get(keyType);
  }
  ...
  public Map<SecurityKeyType, byte[]> getNewKeys(
    ...
    final GetNewSecretsResponse response =
        this.secretManagementClient.getNewSecretsRequest(correlationUid, request);
   ...
   * calling {@link #activateNewKey(String, String, SecurityKeyType)}.
   *
   * @param correlationUid the correlation UID of the original request
   ...
   * String, Map)} after it has been confirmed to be set on the device.
   *
   * @param correlationUid the correlation UID of the original request
   * @param deviceIdentification DLMS device id
   * @param keyType type of key
  ...
  public boolean hasNewSecretOfType(
    ...
    final HasNewSecretResponse response =
        this.secretManagementClient.hasNewSecretRequest(correlationUid, request);
  ...
  public byte[] generate128BitsKeyAndStoreAsNewKey(
    ...
    return this.generate128BitsKeysAndStoreAsNewKeys(
            correlationUid, deviceIdentification, Arrays.asList(keyType))
        .get(keyType);
  }
  ...
  public Map<SecurityKeyType, byte[]> generate128BitsKeysAndStoreAsNewKeys(
    ...
    final GenerateAndStoreSecretsResponse response =
        this.secretManagementClient.generateAndStoreSecrets(correlationUid, request);
