...
public class GetPushSetupAlarmCommandExecutor extends GetPushSetupCommandExecutor<Void, PushSetupAlarmDto> {
    ...
    public PushSetupAlarmDto execute(final DlmsConnectionManager conn, final DlmsDevice device, final Void useless)
        ...
        final List<GetResult> getResultList = DlmsHelper.getWithList(conn, device, ATTRIBUTE_ADDRESSES);
        ...
        pushSetupAlarmBuilder.withPushObjectList(
                DlmsHelper.readListOfObjectDefinition(getResultList.get(INDEX_PUSH_OBJECT_LIST), "Push Object List"));

        pushSetupAlarmBuilder.withSendDestinationAndMethod(DlmsHelper
                .readSendDestinationAndMethod(getResultList.get(INDEX_SEND_DESTINATION_AND_METHOD),
                        "Send Destination And Method"));

        pushSetupAlarmBuilder.withCommunicationWindow(DlmsHelper
                .readListOfWindowElement(getResultList.get(INDEX_COMMUNICATION_WINDOW), "Communication Window"));

        pushSetupAlarmBuilder.withRandomisationStartInterval(DlmsHelper
        ...
        pushSetupAlarmBuilder.withNumberOfRetries(
                DlmsHelper.readLongNotNull(getResultList.get(INDEX_NUMBER_OF_RETRIES), "Number of Retries").intValue());

        pushSetupAlarmBuilder.withRepetitionDelay(
                DlmsHelper.readLongNotNull(getResultList.get(INDEX_REPETITION_DELAY), "Repetition Delay").intValue());
