...
public class DlmsObjectConfigSmr50 extends DlmsObjectConfig {
  ...
  List<DlmsObject> initObjects() {
    ...
    final DlmsObject externalTriggerSmsOrCsd =
        new DlmsAutoAnswer(EXTERNAL_TRIGGER, "0.0.2.2.0.255");
    ...
    final DlmsObject mbusClientSetup =
        new DlmsMbusClientSetup(MBUS_CLIENT_SETUP, "0.<c>.24.1.0.255", VERSION_1);
    ...
    final List<DlmsCaptureObject> captureObjectsDailyE =
        Arrays.asList(
            DlmsCaptureObject.create(clock),
    ...
    objectList.add(
        new DlmsProfile(
            DAILY_LOAD_PROFILE, "1.0.99.2.0.255", captureObjectsDailyE, DAY, ELECTRICITY));

    final List<DlmsCaptureObject> captureObjectsDailyG =
        Arrays.asList(
            DlmsCaptureObject.create(clock),
            ...
            DlmsCaptureObject.create(mbusMasterValue, 5));
    objectList.add(
        new DlmsProfile(DAILY_LOAD_PROFILE, "0.<c>.24.3.1.255", captureObjectsDailyG, DAY, GAS));
    ...
    objectList.add(
        new DlmsProfile(
            MONTHLY_BILLING_VALUES, "1.0.98.1.0.255", captureObjectsMonthly, MONTH, ELECTRICITY));

    final List<DlmsCaptureObject> captureObjectsMonthlyG =
        Arrays.asList(
            DlmsCaptureObject.create(clock),
