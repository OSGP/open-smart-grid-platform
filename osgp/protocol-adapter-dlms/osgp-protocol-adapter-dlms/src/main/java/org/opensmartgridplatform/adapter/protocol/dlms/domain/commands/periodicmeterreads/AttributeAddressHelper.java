...
public class AttributeAddressHelper {
    ...
    private AttributeAddressHelper() {
        throw new IllegalStateException("Utility class");
    }

    public static AttributeAddress[] getProfileBufferAndScalerUnitForPeriodicMeterReads(final PeriodTypeDto periodType,
        ...
        profileBuffer.add(getProfileBuffer(periodType, from, to, isSelectingValuesSupported));
        profileBuffer.addAll(getScalerUnit(periodType));

        return profileBuffer.toArray(new AttributeAddress[0]);
    }

    private static AttributeAddress getProfileBuffer(final PeriodTypeDto periodType, final DateTime from,
            final DateTime to, final boolean isSelectingValuesSupported) throws ProtocolAdapterException {
        final SelectiveAccessDescription access = getSelectiveAccessDescription(periodType, from, to,
    ...
    private static List<AttributeAddress> getScalerUnit(final PeriodTypeDto periodType)
            throws ProtocolAdapterException {
        switch (periodType) {
        case INTERVAL:
            return createScalerUnitForInterval();
        case DAILY:
        case MONTHLY:
            return createScalerUnitForDailyOrMonthly();
        default:
            throw new ProtocolAdapterException(String.format("periodtype %s not supported", periodType));
        }
    }

    private static List<AttributeAddress> createScalerUnitForInterval() {
    ...
    private static List<AttributeAddress> createScalerUnitForDailyOrMonthly() {
        final List<AttributeAddress> scalerUnit = new ArrayList<>();
    ...
    private static SelectiveAccessDescription getSelectiveAccessDescription(final PeriodTypeDto periodType,
            final DateTime from, final DateTime to, final boolean isSelectingValuesSupported) {
        /*
        ...
        if (isSelectingValuesSupported) {
            addSelectedValues(periodType, objectDefinitions);
        }
        final DataObject selectedValues = DataObject.newArrayData(objectDefinitions);

        final DataObject accessParameter = DlmsHelper.getAccessSelectionTimeRangeParameter(from, to, selectedValues);

        return new SelectiveAccessDescription(ACCESS_SELECTOR_RANGE_DESCRIPTOR, accessParameter);
    }

    private static void addSelectedValues(final PeriodTypeDto periodType, final List<DataObject> objectDefinitions) {
        ...
        case INTERVAL:
            ...
            break;
        case DAILY:
            addSelectedValuesForDaily(objectDefinitions);
            break;
        case MONTHLY:
            addSelectedValuesForMonthly(objectDefinitions);
            break;
        default:
    ...
    private static void addSelectedValuesForDaily(final List<DataObject> objectDefinitions) {
        /*-
        ...
        addActiveEnergyImportRate1(objectDefinitions);
        addActiveEnergyImportRate2(objectDefinitions);

        addActiveEnergyExportRate1(objectDefinitions);
        addActiveEnergyExportRate2(objectDefinitions);
    }

    private static void addSelectedValuesForMonthly(final List<DataObject> objectDefinitions) {
        /*-
    ...
    private static void addActiveEnergyImportRate1(final List<DataObject> objectDefinitions) {
        // {3,1-0:1.8.1.255,2,0} - Active energy import (+A) rate 1
    ...
    private static void addActiveEnergyImportRate2(final List<DataObject> objectDefinitions) {
        // {3,1-0:1.8.2.255,2,0} - Active energy import (+A) rate 2
        objectDefinitions.add(DataObject.newStructureData(Arrays.asList(DataObject.newUInteger16Data(CLASS_ID_REGISTER),
    ...
    private static void addActiveEnergyExportRate1(final List<DataObject> objectDefinitions) {
        // {3,1-0:2.8.1.255,2,0} - Active energy export (-A) rate 1
        objectDefinitions.add(DataObject.newStructureData(Arrays.asList(DataObject.newUInteger16Data(CLASS_ID_REGISTER),
    ...
    private static void addActiveEnergyExportRate2(final List<DataObject> objectDefinitions) {
        // {3,1-0:2.8.2.255,2,0} - Active energy export (-A) rate 2
        objectDefinitions.add(DataObject.newStructureData(Arrays.asList(DataObject.newUInteger16Data(CLASS_ID_REGISTER),
