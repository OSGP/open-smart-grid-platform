...
public class GetActualMeterReadsGasCommandExecutor
    ...
    public MeterReadsGasResponseDto execute(final DlmsConnectionManager conn, final DlmsDevice device,
        ...
        final List<GetResult> getResultList = DlmsHelper.getAndCheck(conn, device,
                "retrieve actual meter reads for mbus " + actualMeterReadsRequest.getChannel(), mbusValue, mbusTime,
                scalerUnit);

        final DlmsMeterValueDto consumption = DlmsHelper.getScaledMeterValue(getResultList.get(0), getResultList.get(2),
                "retrieve scaled value for mbus " + actualMeterReadsRequest.getChannel());
        final CosemDateTimeDto cosemDateTime = DlmsHelper.readDateTime(getResultList.get(1), "captureTime gas");
