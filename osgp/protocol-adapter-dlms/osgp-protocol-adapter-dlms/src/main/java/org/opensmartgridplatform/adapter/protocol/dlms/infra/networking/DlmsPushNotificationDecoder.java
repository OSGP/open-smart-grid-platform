...
public class DlmsPushNotificationDecoder extends ReplayingDecoder<DlmsPushNotificationDecoder.DecodingState> {
    ...
    /**
     * Decoded the alarm bytes in the buffer. Could be either a DSMR4 or SMR5
     * alarm. If there are not enough bytes while decoding, the ReplayingDecoder
     * rewinds and tries the decoding again when there are more bytes received.
     *
     * @param ctx
     *            the context from the ReplayingDecoder. Not used in decoding
     *            the alarm.
     * @param in
     *            the bytes of the alarm.
     * @param out
     *            decoded list of objects
