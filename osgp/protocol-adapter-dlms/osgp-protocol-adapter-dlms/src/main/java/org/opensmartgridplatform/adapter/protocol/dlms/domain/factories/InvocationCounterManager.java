...
/**
 * Object that manages initializing and resetting the value of the
 * invocationCounter property of a device.
 */
@Component
public class InvocationCounterManager {
    ...
    @Autowired
    public InvocationCounterManager(final DlmsConnectionFactory connectionFactory, final DlmsHelper dlmsHelper,
    ...
    /**
     * Updates the device instance with the invocation counter value on the
     * actual device. Should only be called for a device that actually has an
     * invocation counter stored on the device itself.
     */
    public void initializeInvocationCounter(final String correlationUid, final DlmsDevice device) throws OsgpException {
        ...
        // At this point proceeding to create a new connection will fail due to
        // a current limitation in the OpenMUC
        // jDLMS library, therefore don't even try but throw a very specific
        // exception signalling the problem.
        // The exception will cause a retry header to be set so the operation
        // will be retried, but the exception
    ...
    private void initializeWithInvocationCounterStoredOnDevice(final String correlationUid, final DlmsDevice device)
