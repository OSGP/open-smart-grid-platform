...
public class Iec61850ClientSSLDEventListener extends Iec61850ClientBaseEventListener {
  ...
  private static final String EVENT_NODE_SWITCH_NUMBER =
      SubDataAttribute.SWITCH_NUMBER.getDescription();
  private static final String EVENT_NODE_SWITCH_VALUE =
      SubDataAttribute.SWITCH_VALUE.getDescription();
  private static final String EVENT_NODE_TRIGGER_TIME =
      SubDataAttribute.TRIGGER_TIME.getDescription();
  private static final String EVENT_NODE_TRIGGER_TYPE =
      SubDataAttribute.TRIGGER_TYPE.getDescription();
  ...
  public Iec61850ClientSSLDEventListener(
    ...
    this.externalIndexByInternalIndex.putAll(
        this.buildExternalByInternalIndexMap(
            this.deviceManagementService, this.deviceIdentification));
  }
  ...
  private Map<Integer, Integer> buildExternalByInternalIndexMap(
    ...
    final List<DeviceOutputSetting> deviceOutputSettings =
        deviceManagementService.getDeviceOutputSettings(deviceIdentification);
    ...
    this.logger.info(
        "Retrieved internal to external index map for device {}: {}",
  ...
  public void newReport(final Report report) {
    ...
    if (Boolean.TRUE.equals(report.getBufOvfl())) {
      this.logger.warn(
          "Buffer Overflow reported for {} - entries within the buffer may have been lost.",
          reportDescription);
    }

    if (this.firstNewSqNum != null
        && report.getSqNum() != null
    ...
    if (CollectionUtils.isEmpty(dataSetMembers)) {
      ...
      this.logger.debug(
          "Handling {} DataSet members for {}", dataSetMembers.size(), reportDescription);
    }
    for (final FcModelNode member : dataSetMembers) {
      ...
      try {
        ...
        this.logger.error(
            "Error adding event notification for member {} from {}",
            member.getReference(),
            reportDescription,
            e);
      }
  ...
  private void addEventNotificationForReportedData(
    ...
    final EventNotificationDto eventNotification =
        new EventNotificationDto(
            this.deviceIdentification, dateTime, eventType, description, index);
    synchronized (this.eventNotifications) {
      this.eventNotifications.add(eventNotification);
    }
  }

  private EventTypeDto determineEventType(
      final FcModelNode evnRpn, final String reportDescription) {
  ...
  private Integer determineRelayIndex(final FcModelNode evnRpn, final String reportDescription) {
    ...
    if (swNumNode == null) {
      throw this.childNodeNotAvailableException(
          evnRpn, EVENT_NODE_SWITCH_NUMBER, reportDescription);
    }
    ...
    if (externalIndex == null) {
      this.logger.error(
          "No external index configured for internal index: {} for device: {}, using '0' for event",
          swNum,
          this.deviceIdentification);
      return 0;
    }
  ...
  private IllegalArgumentException childNodeNotAvailableException(
      final FcModelNode evnRpn, final String childNodeName, final String reportDescription) {
    return new IllegalArgumentException(
        "No '"
            + childNodeName
            + "' child in DataSet member "
            + evnRpn.getReference()
            + " from "
            + reportDescription);
  }
  ...
  private void checkNodeType(
      final FcModelNode member, final StringBuilder sb, final String nodeType) {
  ...
  private void logEventNotificationDeviceMessage(final FcModelNode evnRpn) {
    final DeviceMessageLog deviceMessageLog =
        new DeviceMessageLog(IED.FLEX_OVL, LogicalDevice.LIGHTING, "EventNotification");

    final BdaInt8U evnTypeNode = (BdaInt8U) evnRpn.getChild(EVENT_NODE_EVENT_TYPE);
    if (evnTypeNode != null) {
      deviceMessageLog.addVariable(
          LogicalNode.STREET_LIGHT_CONFIGURATION,
          DataAttribute.EVENT_RPN,
          evnRpn.getFc(),
          SubDataAttribute.EVENT_TYPE,
          Short.toString(evnTypeNode.getValue()));
    }
    ...
    if (swNumNode != null) {
      deviceMessageLog.addVariable(
          ...
          DataAttribute.EVENT_RPN,
          evnRpn.getFc(),
          SubDataAttribute.SWITCH_NUMBER,
          Short.toString(swNumNode.getValue()));
    }
    ...
    if (trgTypeNode != null) {
      deviceMessageLog.addVariable(
          ...
          DataAttribute.EVENT_RPN,
          evnRpn.getFc(),
          SubDataAttribute.TRIGGER_TYPE,
          Short.toString(trgTypeNode.getValue()));
    }
    ...
    if (swValNode != null) {
      deviceMessageLog.addVariable(
          ...
          DataAttribute.EVENT_RPN,
          evnRpn.getFc(),
          SubDataAttribute.SWITCH_VALUE,
          swValNode.getValueString());
    }
    ...
    if (trgTimeNode != null) {
      deviceMessageLog.addVariable(
          ...
          DataAttribute.EVENT_RPN,
          evnRpn.getFc(),
          SubDataAttribute.TRIGGER_TIME,
          trgTimeNode.getDate().toString());
    }
    ...
    if (remarkNode != null) {
      deviceMessageLog.addVariable(
          ...
          DataAttribute.EVENT_RPN,
          evnRpn.getFc(),
          SubDataAttribute.REMARK,
          remarkNode.getStringValue());
    }

    this.loggingService.logMessage(
        deviceMessageLog, this.deviceIdentification, this.organizationIdentification, true);
  }
  ...
  public void associationClosed(final IOException e) {
    ...
    synchronized (this.eventNotifications) {
      if (this.eventNotifications.isEmpty()) {
        this.logger.info(
            "No event notifications received from device: {}", this.deviceIdentification);
        return;
      }
      ...
      try {
        this.deviceManagementService.addEventNotifications(
            this.deviceIdentification, this.eventNotifications);
      } catch (final ProtocolAdapterException pae) {
        this.logger.error(
            "Error adding device notifications for device: " + this.deviceIdentification, pae);
      }
    }
  }
}
