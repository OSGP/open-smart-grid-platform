...
public abstract class Iec61850ClientBaseEventListener implements ClientEventListener {
  ...
  public Iec61850ClientBaseEventListener(
  ...
  /**
   * Before enabling reporting on the device, set the SqNum of the buffered report data to be able
   * to check if incoming reports have been received already.
   *
   * @param value the value of SqNum of a BR node on the device.
   */
  public void setSqNum(final int value) {
    this.logger.info(
        "First new SqNum for report listener for device: {} is: {}",
  ...
  /**
   * The logging of the {@link Report} consists of a default part and a custom part. This method is
   * intended for the default part.
   */
  public void logDefaultReportDetails(final StringBuilder sb, final Report report) {
    ...
    sb.append("\tInclusionBitString:\t")
        ...
        .append(System.lineSeparator());
    sb.append("\tMoreSegmentsFollow:\t")
        .append(report.isMoreSegmentsFollow())
        .append(System.lineSeparator());
    sb.append("\t             SqNum:\t").append(report.getSqNum()).append(System.lineSeparator());
    sb.append("\t          SubSqNum:\t")
        .append(report.getSubSqNum())
        .append(System.lineSeparator());
    sb.append("\t       TimeOfEntry:\t")
        .append(report.getTimeOfEntry())
        .append(System.lineSeparator());
    ...
    if ((reasonCodes != null) && !reasonCodes.isEmpty()) {
      ...
      for (final BdaReasonForInclusion reasonCode : reasonCodes) {
        sb.append("\t                   \t")
            .append(
                reasonCode.getReference() == null
                    ? HexConverter.toHexString(reasonCode.getValue())
                    : reasonCode)
            .append("\t(")
