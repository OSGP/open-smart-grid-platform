...
public class Iec61850ChannelHandlerServer extends Iec61850ChannelHandler {
  ...
  @Override
  public void channelRead0(final ChannelHandlerContext ctx, final RegisterDeviceRequest message)
      throws Exception {
  ...
  private void processRegistrationMessage(
      final RegisterDeviceRequest message, final String correlationId) {
    ...
    if (this.deviceRegistrationService.isKnownDevice(deviceIdentification)) {
      LOGGER.info(
          "Device {} found, start processing this registration message", deviceIdentification);
    } else {
      LOGGER.warn(
          "Ignoring this registration message, because there's no device having identification {}",
    ...
    final DeviceRegistrationDataDto deviceRegistrationData =
        new DeviceRegistrationDataDto(ipAddress, Ssld.SSLD_TYPE, true);
    ...
    try {
      this.deviceRegistrationService.disableRegistration(
          deviceIdentification, InetAddress.getByName(ipAddress), ied, ied.getDescription());

      final RequestMessage cdrRequestMessage =
          new RequestMessage(correlationId, NO_ORGANISATION, deviceIdentification, ipAddress);

      this.osgpRequestMessageSender.send(
          cdrRequestMessage, MessageType.DEVICE_REGISTRATION_COMPLETED.name());
      LOGGER.info(
          "Disabled registration for device: {}, at IP address: {}",
          deviceIdentification,
          ipAddress);
    } catch (final Exception e) {
      LOGGER.error(
          "Failed to disable registration for device: {}, at IP address: {}",
          deviceIdentification,
          ipAddress,
          e);
    }
