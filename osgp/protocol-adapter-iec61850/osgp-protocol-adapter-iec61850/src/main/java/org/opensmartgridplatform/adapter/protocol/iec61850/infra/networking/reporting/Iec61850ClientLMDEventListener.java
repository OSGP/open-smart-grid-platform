...
public class Iec61850ClientLMDEventListener extends Iec61850ClientBaseEventListener {
  ...
  public Iec61850ClientLMDEventListener(
  ...
  public void newReport(final Report report) {
    ...
    if (this.firstNewSqNum != null
      ...
      this.logger.warn(
          "report.getSqNum() < this.firstNewSqNum, report.getSqNum() = {}, this.firstNewSqNum = {}",
          report.getSqNum(),
          this.firstNewSqNum);
    }
    ...
    final Map<LightMeasurementDevice, FcModelNode> reportMemberPerDevice =
        this.processReportedDataForLightMeasurementDevices(report.getValues());
    ...
    for (final LightMeasurementDevice lmd : reportMemberPerDevice.keySet()) {
      ...
      final EventNotificationDto eventNotification =
          this.getEventNotificationForReportedData(
              member, timeOfEntry, reportDescription, deviceIdentification, index.intValue());

      try {
        this.deviceManagementService.addEventNotifications(
            deviceIdentification, Arrays.asList(eventNotification));
      } catch (final ProtocolAdapterException pae) {
        this.logger.error(
            "Error adding device notifications for device: " + deviceIdentification, pae);
      }
    }
  }
  ...
  private Map<LightMeasurementDevice, FcModelNode> processReportedDataForLightMeasurementDevices(
    ...
    final List<LightMeasurementDevice> lmds =
        this.deviceManagementService.findRealLightMeasurementDevices();
    this.logger.info("Found {} light measurement devices.", lmds == null ? "null" : lmds.size());

    for (final LightMeasurementDevice lmd : lmds) {
      final String nodeName =
          LogicalNode.getSpggioByIndex(lmd.getDigitalInput()).getDescription().concat(".");
  ...
  private EventNotificationDto getEventNotificationForReportedData(
    ...
    return new EventNotificationDto(
        deviceIdentification, timeOfEntry, eventType, reportDescription, index);
  }

  private boolean determineLightSensorValue(
