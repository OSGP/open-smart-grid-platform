...
class OslpDecoderTest {
    ...
    void decodesAnOslpEnvelopeEncodedByOslpEncoder() {
        final EmbeddedChannel channel = new EmbeddedChannel(new InboundOslpEncoder(), this.oslpDecoder());
        final OslpEnvelope oslpEnvelope = this.oslpEnvelope();
    ...
    private OslpEnvelope oslpEnvelope() {
        return new OslpEnvelope.Builder().withSignature(this.signatureDefinition.algorithm())
    ...
    private KeyPair randomKeyPair(final SignatureDefinition signatureDefinition) throws GeneralSecurityException {
    ...
    private byte[] randomDeviceId() {
        final byte[] randomDeviceUid = new byte[OslpEnvelope.MANUFACTURER_ID_LENGTH + OslpEnvelope.DEVICE_ID_LENGTH];
    ...
    private String oslpEnvelopeDetails(final OslpEnvelope oslpEnvelope, final SignatureDefinition signatureDefinition,
        ...
        return String.format(
                ...
                algorithmDetails, providerDetails, privateKeyDetails, oslpEnvelope.getSize(), securityKeyDetails,
                sequenceNumberDetails, deviceIdDetails, lengthIndicatorDetails, oslpEnvelope.getPayloadMessage());
    }
    ...
    private String deviceIdDetails(final byte[] deviceId) {
        ...
        return String.format("%s (Hex), %s (Base64)", Hex.encodeHexString(deviceId),
                Base64.getEncoder().encodeToString(deviceId));
    }
    ...
    private String privateKeyDetails(final PrivateKey privateKey) {
        ...
        return String.format("algorithm = %s, format = %s, encoded = %s", privateKey.getAlgorithm(),
                privateKey.getFormat(), this.byteArrayDetails(privateKey.getEncoded()));
    }
    ...
    enum OslpMessageType {
        REGISTER_DEVICE_REQUEST {
            ...
            public Message randomMessage(final OslpDecoderTest test) {
                return Message.newBuilder()
                        .setRegisterDeviceRequest(RegisterDeviceRequest.newBuilder()
        ...
        CONFIRM_REGISTER_DEVICE_REQUEST {
            ...
            public Message randomMessage(final OslpDecoderTest test) {
                return Message.newBuilder()
                        .setConfirmRegisterDeviceRequest(ConfirmRegisterDeviceRequest.newBuilder()
                                .setRandomDevice(test.randomDeviceOrPlatform())
        ...
        EVENT_NOTIFICATION_REQUEST {
            private final DateTimeFormatter oslpTimestampFormatter = DateTimeFormatter.ofPattern("yyyyMMddHHmmss");
