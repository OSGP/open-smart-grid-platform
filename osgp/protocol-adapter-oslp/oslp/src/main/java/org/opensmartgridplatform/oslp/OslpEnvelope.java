...
public class OslpEnvelope implements Serializable {
  ...
  private static final String SECURITY_CONFIG_EXCEPTION =
      "Given security settings caused an error.";

  /**
   * Length of the security hash NOTE: length for ECDSA could be smaller (73), but leave it for now
   * on 128 for testing with AME.
   */
  ...
  /** Indicates whether message is valid. Only available after validate method has been called. */
   ...
   * @param privateKey privatekey for signing
   * @param securityKey securitykey containing validation bytes
    ...
    if (newSecurityKey == null) {
      ...
      if (newSecurityKey.length != SECURITY_KEY_LENGTH) {
        throw new IllegalArgumentException(
            "SecurityKey is not of expected Length: " + SECURITY_KEY_LENGTH);
      }
  ...
  /** Set new device id and new manufacturer id. Both values will be combined into one array. */
  public void setDeviceId(final byte[] newDeviceId) {
    ...
    if (newDeviceId == null) {
      ...
      if (newDeviceId.length != (DEVICE_ID_LENGTH + MANUFACTURER_ID_LENGTH)) {
        throw new IllegalArgumentException(
            "ManufacturerId + DeviceId is not of expected Length: "
  ...
  /**
   * Validates the envelope + payload (excluding securityKey) using signature algorithm.
   *
   * @return true when securityKey and calculated signature match or false otherwise.
   */
  public boolean validate(final PublicKey publicKey) {
    try {
      this.valid =
          OslpUtils.validateSignature(
              OslpUtils.createSignBytes(this),
              this.getSecurityKey(),
              publicKey,
              this.signature,
              this.provider);
  ...
  /**
   * Calculate the new securityKey based on envelope + payload (excluding securityKey) using
   * signature algorithm.
   */
  private void setSecurityKey() {
    try {
      ...
      final byte[] sig =
          OslpUtils.createSignature(
              OslpUtils.createSignBytes(this), this.privateKey, this.signature, this.provider);
  ...
  public boolean equals(final Object o) {
    ...
    if (this.payloadMessage != null
        ? !this.payloadMessage.equals(that.payloadMessage)
        : that.payloadMessage != null) {
      return false;
    }
    if (this.privateKey != null
        ? !this.privateKey.equals(that.privateKey)
        : that.privateKey != null) {
      return false;
    }
