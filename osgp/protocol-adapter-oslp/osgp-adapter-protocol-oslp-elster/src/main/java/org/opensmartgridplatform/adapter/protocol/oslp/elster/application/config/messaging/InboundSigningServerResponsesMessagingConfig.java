...
public class InboundSigningServerResponsesMessagingConfig {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(InboundSigningServerResponsesMessagingConfig.class);
  ...
  public InboundSigningServerResponsesMessagingConfig(
    ...
    this.jmsConfigurationFactory =
        new JmsConfigurationFactory(
            environment, defaultJmsConfiguration, "jms.incoming.signing.server.responses");
  }

  @Bean(destroyMethod = "stop", name = "protocolOslpInboundSigningServerResponsesConnectionFactory")
  public ConnectionFactory connectionFactory() {
    LOGGER.info(
        "Initializing protocolOslpInboundSigningServerResponsesJmsConfigurationFactory bean.");
  ...
  public DefaultMessageListenerContainer messageListenerContainer(
    ...
    LOGGER.info(
        "Initializing protocolOslpInboundSigningServerResponsesMessageListenerContainer bean.");
    return this.jmsConfigurationFactory.initMessageListenerContainer(
        messageListener, this.replyToQueue());
  }

  /**
   * Instead of a fixed name for the responses queue, the signing-server uses a 'reply-to' responses
   * queue. This 'reply-to' responses queue is communicated to the signing-server by this
   * Protocol-Adapter-OSLP instance when a request message is sent to the signing-server. The
   * signing-server will send signed response messages to the 'reply-to' queue. This ensures that
   * the signed response messages for this Protocol-Adapter-OSLP instance are sent back to this
   * instance.
   */
  @Bean
