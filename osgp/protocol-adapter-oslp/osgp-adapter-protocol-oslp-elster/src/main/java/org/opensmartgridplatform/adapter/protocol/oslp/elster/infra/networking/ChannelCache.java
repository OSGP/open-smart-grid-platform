...
public class ChannelCache {
    ...
    public void cacheChannel(final Channel channel) {
        ...
        if (referenceCount == 1) {
            ...
            LOGGER.info("Cached channel {} multiple times (reference count: {})", channelIdAsLongText, referenceCount);
        }
    ...
    public Channel removeFromCache(final String channelIdAsLongText) {
        ...
        if (referenceCount == 0) {
            ...
            LOGGER.info("Keep channel {} in cache (remaining reference count: {})", channelIdAsLongText,
                    referenceCount);
        }
    ...
    public void removeExpiredCacheEntries() {
        ...
        synchronized (this) {
            keysForExpiredEntries.forEach(channelIdAsLongText -> {
                ...
                if (removed != null && removed.getLastRefreshedMillis() > thresholdMillis) {
                    ...
                    LOGGER.warn("Removed channel {} from cache after expiration: {}", channelIdAsLongText, removed);
                }
            });
        }
