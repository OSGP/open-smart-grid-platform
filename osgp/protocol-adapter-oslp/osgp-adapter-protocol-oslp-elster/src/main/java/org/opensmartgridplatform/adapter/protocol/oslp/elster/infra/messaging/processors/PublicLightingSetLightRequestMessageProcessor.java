...
public class PublicLightingSetLightRequestMessageProcessor extends DeviceRequestMessageProcessor
  ...
  @Autowired
  private PublicLightingResumeScheduleRequestMessageProcessor
      publicLightingResumeScheduleRequestMessageProcessor;
  ...
  public void processMessage(final ObjectMessage message) {
    ...
    try {
      ...
      final SetLightDeviceRequest deviceRequest =
          new SetLightDeviceRequest(
  ...
  public void processSignedOslpEnvelope(
        ...
        new DeviceResponseHandler() {
          ...
          public void handleResponse(final DeviceResponse deviceResponse) {
            if (((EmptyDeviceResponse) deviceResponse).getStatus().equals(DeviceMessageStatus.OK)) {
              ...
              LOGGER.info(
                  "setLight() successful for device : {}",
                  deviceResponse.getDeviceIdentification());
            } else {
              // If the response is not OK, send a response message to the
              // responses queue.
              PublicLightingSetLightRequestMessageProcessor.this.handleEmptyDeviceResponse(
                  deviceResponse,
    ...
    // = 0' as arguments.
    final ResumeScheduleMessageDataContainerDto resumeScheduleMessageDataContainer =
        new ResumeScheduleMessageDataContainerDto(0, false);

    final DeviceResponseHandler resumeScheduleDeviceResponseHandler =
        this.publicLightingResumeScheduleRequestMessageProcessor
            .createResumeScheduleDeviceResponseHandler(
                domain, domainVersion, MessageType.RESUME_SCHEDULE.name(), retryCount, isScheduled);

    final ResumeScheduleDeviceRequest resumeScheduleDeviceRequest =
        new ResumeScheduleDeviceRequest(
            DeviceRequest.newBuilder()
                .organisationIdentification(organisationIdentification)
                .deviceIdentification(deviceIdentification)
                .correlationUid(correlationUid)
                .domain(domain)
                .domainVersion(domainVersion)
                .messageType(MessageType.RESUME_SCHEDULE.name())
                .messagePriority(messagePriority)
                .ipAddress(ipAddress)
                .retryCount(retryCount)
                .isScheduled(isScheduled),
