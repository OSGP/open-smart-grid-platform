...
public class OslpSigningService {
  ...
  @Autowired private SigningServerRequestMessageSender signingServerRequestMessageSender;
  ...
  /**
   * Build OslpEnvelope for an OSLP request using the arguments supplied and have the envelope
   * signed by the signing server.
   */
  public void buildAndSignEnvelope(
    ...
    final UnsignedOslpEnvelopeDto oslpEnvelopeDto =
        new UnsignedOslpEnvelopeDto(
            sequenceNumber,
            deviceId,
            payloadMessage,
            ipAddress,
    ...
    final RequestMessage requestMessage =
        new RequestMessage(
            correlationUid, organisationIdentification, deviceIdentification, oslpEnvelopeDto);

    // Send request message to signing server.
    this.signingServerRequestMessageSender.send(
        requestMessage, SIGNING_REQUEST_MESSAGE_TYPE, messagePriority);
  }

  /**
   * Build OslpEnvelope for an OSLP response using the arguments supplied and have the envelope
   * signed by the signing server.
   */
  public void buildAndSignEnvelope(
      final byte[] deviceId,
    ...
    final UnsignedOslpEnvelopeDto unsignedOslpEnvelopeDto =
        new UnsignedOslpEnvelopeDto(sequenceNumber, deviceId, payloadMessage, correlationUid);
    final RequestMessage requestMessage =
        new RequestMessage(
            correlationUid,
            "organisationIdentification",
  ...
  public void handleSignedOslpEnvelope(
  ...
  private void handleSignedOslpRequest(
    ...
    LOGGER.debug(
        ...
        unsignedOslpEnvelopeDto.getCorrelationUid());
    LOGGER.debug(
        "unsignedOslpEnvelopeDto.getDeviceId() : {}", unsignedOslpEnvelopeDto.getDeviceId());
    LOGGER.debug("unsignedOslpEnvelopeDto.getDomain() : {}", unsignedOslpEnvelopeDto.getDomain());
    LOGGER.debug(
        "unsignedOslpEnvelopeDto.getDomainVersion() : {}",
        unsignedOslpEnvelopeDto.getDomainVersion());
    LOGGER.debug(
        "unsignedOslpEnvelopeDto.getIpAddress() : {}", unsignedOslpEnvelopeDto.getIpAddress());
    LOGGER.debug(
        "unsignedOslpEnvelopeDto.getMessageType() : {}", unsignedOslpEnvelopeDto.getMessageType());
    LOGGER.debug(
    ...
    LOGGER.debug(
        "unsignedOslpEnvelopeDto.getRetryCount() : {}", unsignedOslpEnvelopeDto.getRetryCount());
    LOGGER.debug(
        "unsignedOslpEnvelopeDto.getSequenceNumber() : {}",
        unsignedOslpEnvelopeDto.getSequenceNumber());
    LOGGER.debug(
        "unsignedOslpEnvelopeDto.isScheduled() : {}", unsignedOslpEnvelopeDto.isScheduled());
    LOGGER.debug(LINES);

    final MessageType messageType = MessageType.valueOf(unsignedOslpEnvelopeDto.getMessageType());

    // Handle message for message type.
    final OslpEnvelopeProcessor messageProcessor =
        this.deviceRequestMessageProcessorMap.getOslpEnvelopeProcessor(messageType);
    if (messageProcessor == null) {
      LOGGER.error(
          "No message processor for messageType: {}", unsignedOslpEnvelopeDto.getMessageType());
      return;
    }
  ...
  public void handleError(
      final String deviceIdentification, final ResponseMessage responseMessage) {
