...
public class OslpSecurityHandler extends SimpleChannelInboundHandler<OslpEnvelope> {
  ...
  @Autowired private String oslpKeyType;
  ...
  public void channelRead0(final ChannelHandlerContext ctx, final OslpEnvelope message)
    ...
    if (message.getPayloadMessage().hasRegisterDeviceRequest()) {
      final String deviceIdentification =
          message.getPayloadMessage().getRegisterDeviceRequest().getDeviceIdentification();

      oslpDevice =
          this.oslpDeviceSettingsService.getDeviceByDeviceIdentification(deviceIdentification);
    } else {
      oslpDevice =
          this.oslpDeviceSettingsService.getDeviceByUid(
              Base64.encodeBase64String(message.getDeviceId()));
    }
    ...
    if (oslpDevice == null) {
      ...
      LOGGER.warn(
          "Received message from device without public key: {}",
    ...
    if (oslpDevice != null && oslpDevice.getPublicKey() != null) {
      final PublicKey publicKey =
          CertificateHelper.createPublicKeyFromBase64(
              oslpDevice.getPublicKey(), this.oslpKeyType, this.oslpSignatureProvider);
