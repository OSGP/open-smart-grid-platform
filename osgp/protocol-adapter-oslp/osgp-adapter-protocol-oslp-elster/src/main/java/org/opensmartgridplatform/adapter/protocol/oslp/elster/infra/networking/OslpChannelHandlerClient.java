...
public class OslpChannelHandlerClient extends OslpChannelHandler {
  ...
  @Autowired private String successfulMessagesMetric;
  ...
  public void channelRead0(final ChannelHandlerContext ctx, final OslpEnvelope message) {
    ...
    if (message.isValid()) {
      if (OslpUtils.isOslpResponse(message)) {
        LOGGER.info(
            "{} Received OSLP Response (before callback): {}",
            channelId,
            message.getPayloadMessage());

        // Check the sequence number
        final Integer sequenceNumber =
            SequenceNumberUtils.convertByteArrayToInteger(message.getSequenceNumber());
        ...
        LOGGER.warn(
            "{} Received OSLP Request, which is not expected: {}",
            channelId,
            message.getPayloadMessage());
      }
    } else {
  ...
  public void send(
    ...
    this.callbackHandlers.put(
        channelFuture.channel().id().asLongText(), new OslpCallbackHandler(responseHandler));
        ...
        new ChannelFutureListener() {
          ...
          public void operationComplete(final ChannelFuture future) throws IOException {
            ...
            if (future.isSuccess()) {
              ...
              LOGGER.info(
                  "The connection to the device {} is not successful", deviceIdentification);
  ...
  private void write(
    ...
    if (channel != null && channel.isActive()) {
      LOGGER.info(
          "Channel [{}] - Connection established to: {}", channelFuture.channel().id(), address);
    } else {
    ...
    try {
      ...
      LOGGER.error(
          "{} Exception while writing request: {}", channelFuture.channel().id(), e.getCause(), e);
