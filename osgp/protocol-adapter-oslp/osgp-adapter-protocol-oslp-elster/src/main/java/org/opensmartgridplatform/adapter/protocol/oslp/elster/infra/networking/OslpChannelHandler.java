...
public abstract class OslpChannelHandler extends SimpleChannelInboundHandler<OslpEnvelope> {
  ...
  @Resource protected String oslpSignatureProvider;

  @Resource protected String oslpSignature;

  @Resource protected int connectionTimeout;

  protected final ConcurrentMap<String, OslpCallbackHandler> callbackHandlers =
  ...
  private void acquireConnection() throws InterruptedException {
    if (OslpChannelHandler.limitConnections) {
      ...
      this.logger.info(
          "Connection requested. Available connections: "
              + availableConnections.availablePermits());

      availableConnections.acquire();

      final Duration acquireDuration = Duration.between(acquireStart, Instant.now());
      this.logger.info(
          "Connection granted. Available connections: {}, acquireDuration (ms) {}",
          availableConnections.availablePermits(),
          acquireDuration.toMillis());
    }
  }
  ...
  private void releaseConnection() {
    if (OslpChannelHandler.limitConnections) {
      this.logger.info(
          "Connection released. Available connections: " + availableConnections.availablePermits());
  ...
  public void exceptionCaught(final ChannelHandlerContext ctx, final Throwable cause)
    ...
    if (this.isConnectionReset(cause)) {
      this.logger.info(
          "Channel [{}] - Connection was (as expected) reset by the device.", channelId);
    } else {
