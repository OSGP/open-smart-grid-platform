...
public class LightMeasurementDeviceConfig {

    private static final Logger LOGGER = LoggerFactory.getLogger(LightMeasurementDeviceConfig.class);

    @Value("${job.asdu.generator.cron:0 0/1 * * * ?}")
    private String cronExpression;

    @Bean
    public Iec60870AsduGenerator lightMeasurementEventAsduGenerator() {
        return new LightMeasurementEventAsduGenerator();
    }

    /**
     * Bean used to generate light measurement events for testing purposes. By
     * default no light measurement events will be generated. To activate
     * generation, add job.asdu.generator.enabled to the application's
     * properties.
     */
    @Bean
    public Iec60870AsduGeneratorService lightMeasurementEventAsduGeneratorService(
            final Iec60870ConnectionRegistry iec60870ConnectionRegistry) {
        LOGGER.info("LightMeasurement ASDU generator in simulator is enabled");

        final CronTrigger cronTrigger = new CronTrigger(this.cronExpression, TimeZone.getTimeZone("UTC"));
        final Iec60870AsduGenerator generator = this.lightMeasurementEventAsduGenerator();
