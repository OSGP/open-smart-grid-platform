...
public class LightMeasurementDeviceAsduFactory implements Iec60870AsduFactory {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(LightMeasurementDeviceAsduFactory.class);
    ...
    public ASdu createLightMeasurementEvent() {
        ...
        LOGGER.info("Creating Light Measurement event for IOA {} with value {} ({})", this.ioa[index], this.iev[index],
                eventValue);

        informationObjects[0] = new InformationObject(this.ioa[index],
                this.createInformationElementWithTimetag(this.iev[index], timestamp));

        return new Iec60870AsduBuilder().withAsduType(ASduType.M_SP_TB_1)
                .withSequenceOfElements(false)
