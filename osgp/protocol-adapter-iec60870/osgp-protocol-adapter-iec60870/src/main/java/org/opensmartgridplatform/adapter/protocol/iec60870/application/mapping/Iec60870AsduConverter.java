...
public class Iec60870AsduConverter extends CustomConverter<ASdu, MeasurementReportDto> {
    ...
    @Override
    public MeasurementReportDto convert(final ASdu source, final Type<? extends MeasurementReportDto> destinationType,
            final MappingContext mappingContext) {

        return new MeasurementReportDto(new MeasurementReportHeaderDto(source.getTypeIdentification().name(),
                source.getCauseOfTransmission().name(), source.getOriginatorAddress(), source.getCommonAddress()),
