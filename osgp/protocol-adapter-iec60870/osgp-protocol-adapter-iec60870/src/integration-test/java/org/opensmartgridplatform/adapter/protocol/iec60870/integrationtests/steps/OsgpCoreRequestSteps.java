...
public class OsgpCoreRequestSteps {
  ...
  @Autowired private DeviceRequestMessageListener messageListener;
  ...
  public void whenIReceiveAConnectRequestForIEC60870DeviceFromOsgpCore(
  ...
  @When(
      "I receive a get light sensor status request message for IEC60870 device {string} from osgp core")
  ...
  public void theProtocolAdapterShouldSendALightMeasurementEventToOsgpCore(
      final Map<String, String> eventData) {

    final String expectedEventType = Objects.requireNonNull(eventData.get("event_type"));
    final String expectedDeviceIdentification =
        Objects.requireNonNull(eventData.get("device_identification"));

    final ArgumentCaptor<RequestMessage> requestMessageCaptor =
        ArgumentCaptor.forClass(RequestMessage.class);
    verify(this.osgpRequestMessageSenderMock, times(1))
        .send(requestMessageCaptor.capture(), eq(MessageType.EVENT_NOTIFICATION.name()));
