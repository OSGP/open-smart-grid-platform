...
public enum PingFlavor {
    ...
    LINUX("ping", "-c %d", "-s %d", "-W %d", Duration::getSeconds, lookupNames -> Boolean.FALSE.equals(lookupNames)
            ? "-n"
                    : ""),
    MAC_OS("ping", "-c %d", "-s %d", "-t %d", Duration::getSeconds, lookupNames -> Boolean.FALSE.equals(lookupNames)
            ? "-n"
                    : ""),
    WINDOWS("ping", "-n %d", "-l %d", "-w %d", Duration::toMillis, lookupNames -> Boolean.TRUE.equals(lookupNames)
            ? "-a"
                    : "");
    ...
    PingFlavor(final String pingCommand, final String countFlagFormat, final String sizeFlagFormat,
            final String timeoutFlagFormat, final ToLongFunction<Duration> timeoutValue,
    ...
    public String timeoutFlag(final Duration timeout) {
        if (this.timeoutFlagFormat == null || timeout == null || this.timeoutValue.applyAsLong(timeout) < 1) {
            return "";
        }
