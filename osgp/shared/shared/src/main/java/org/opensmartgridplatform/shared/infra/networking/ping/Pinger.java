...
public class Pinger {
    ...
    public Pinger(final int count, final int size, final Duration timeout, final Boolean lookupNamesForHostAddresses) {
        this(count, size, timeout, lookupNamesForHostAddresses, new CommandExecutor());
    }

    Pinger(final int count, final int size, final Duration timeout, final Boolean lookupNamesForHostAddresses,
    ...
    public boolean ping(final String destination) {
        final CommandBuilder commandBuilder = new CommandBuilder().withCount(this.count)
                .withSize(this.size)
        ...
        try {
            ...
            final Predicate<String> probableEchoReplyWithTtl = s -> s.toLowerCase(Locale.UK).contains("ttl=");
            final boolean pingInputLikelyHasEchoReplies = inputLines.stream().anyMatch(probableEchoReplyWithTtl);
