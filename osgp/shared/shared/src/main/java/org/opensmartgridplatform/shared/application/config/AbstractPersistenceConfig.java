 ...
 * Configuring the datasource URL can be done in two ways:
 * <ul>
 *     <li><code>db.url</code> with the complete URL. If this property is present it will be used</li>
 *     <li>Separate properties for <code>db.protocol</code>, <code>db.host</code>, <code>db.port</code> and <code>db.name</code></li>
 * </ul>
 * You can also combine it when using several databases. Define a global property e.g. <code>db.url=jdbc:postgres://host1:5432,host2:5432/${db.name}</code>.
 * This way you can define <code>db.host</code> per app, but have a global host config.
 */
