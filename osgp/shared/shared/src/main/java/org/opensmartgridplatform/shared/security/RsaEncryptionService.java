...
/**
 * {@link RsaEncryptionService} provides methods for encrypting and/or decrypting based on {@value
 * #ALGORITHM} {@link RSAPublicKey public} and {@link RSAPrivateKey private} keys. It uses a cipher
 * with transformation {@value #TRANSFORMATION}.
 *
 * <p>The service is only able to {@link #encrypt(byte[]) encrypt bytes} if a public key is
 * configured for property key {@code encryption.rsa.public.key.path}, which should be a path to a
 * readable file.<br>
 * This public key file should contain binary data according to the {@link X509EncodedKeySpec X.509
 * encoded key spec}.
 *
 * <p>The service is only able to {@link #decrypt(byte[]) decrypt bytes} if a private key is
 * configured for property key {@code encryption.rsa.private.key.path}, which should be a path to a
 * readable file.<br>
 * This private key file should contain binary data according to the {@link PKCS8EncodedKeySpec
 * PKCS#8 encoded key spec}.
 */
@Component
public class RsaEncryptionService {
  ...
  private static final String UNEXPECTED_EXCEPTION_DURING_ENCRYPTION =
      "Unexpected exception during encryption";
  private static final String UNEXPECTED_EXCEPTION_DURING_DECRYPTION =
   ...
   *
   * <p>If the public key in the keyPair is {@code null}, encryption will not be available.<br>
   * If the private key in the keyPair is {@code null}, decryption will not be available.
   *
   * @param keyPair a key pair for algorithm {@value #ALGORITHM}.
   */
  ...
  /**
   * Note that the {@link RsaEncryptionService} may be used in a place where the public key of the
   * key pair in use is not configured. In such a case a call to this method will result in an
   * exception.
   *
   * @param input bytes to be encrypted.
   * @return bytes encrypted with the public key of the configured key pair.
   * @throws EncrypterException if the public key is not configured or if anything goes wrong while
   *     encrypting the given {@code input}.
   */
  ...
  /**
   * Note that the {@link RsaEncryptionService} may be used in a place where the private key of the
   * configured key pair is not available. In such a case a call to this method will result in an
   * exception.
   *
   * @param input bytes encrypted with the public key of the configured key pair.
   * @return bytes decrypted with the private key of the configured key pair.
   * @throws EncrypterException if the private key is not configured or if anything goes wrong while
   *     decrypting the given {@code input}.
   */
  ...
  /**
   * Convenience method to read a public key from a file with the given {@code filename}.
   *
   * <p>If this method returns a usable key for a certain file, the {@link RsaEncryptionService}
   * should be able to use that key for {@link #encrypt(byte[]) encryption} if the filename is
   * configured as property key {@code encryption.rsa.public.key.path}.
   *
   * @param filename the name of a file containing binary data according to the {@link
   *     X509EncodedKeySpec X.509 encoded key spec}.
   * @return a public key for algorithm {@value #ALGORITHM}, or {@code null} if the given {@code
   *     filename} is blank.
   */
  ...
  /**
   * Convenience method to read a private key from a file with the given {@code filename}.
   *
   * <p>If this method returns a usable key for a certain file, the {@link RsaEncryptionService}
   * should be able to use that key for {@link #decrypt(byte[]) decryption} if the filename is
   * configured as property key {@code encryption.rsa.private.key.path}.
   *
   * @param filename the name of a file containing binary data according to the {@link
   *     PKCS8EncodedKeySpec PKCS#8 encoded key spec}.
   * @return a public key for algorithm {@value #ALGORITHM}, or {@code null} if the given {@code
   *     filename} is blank.
   */
  ...
  /**
   * Convenience method to create a new keypair for algorithm {@value #ALGORITHM} and the given
   * {@code keysize}.
   *
   * @param keysize the keysize for which the key pair generator will be initialized.
   * @return a randomly initialized {@value #ALGORITHM} key pair for the given {@code keysize}.
   */
  ...
  /**
   * Convenience method to store a key to a file with the given {@code filename}.
   *
   * <p>If the given key is for the correct algorithm, the file should be usable as input for
   * reading a {@link #readPublicKeyFromFile(String) public} or {@link
   * #readPrivateKeyFromFile(String) private} key.
   *
   * @param key a key implementation for algorithm {@value #ALGORITHM}.
   * @param filename the name of a file to be newly created.
   * @throws IOException if a file with the given {@code filename} already exists or cannot be
