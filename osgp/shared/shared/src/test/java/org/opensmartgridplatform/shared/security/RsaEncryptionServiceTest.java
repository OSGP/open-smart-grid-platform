...
public class RsaEncryptionServiceTest {
  ...
  public void testDecryptionOfKeys() throws Exception {
    ...
    this.assertDecryptionOfKey(
        rsaEncryptionService, MASTER_KEY_ENCRYPTED_HEX_STRING, MASTER_KEY_PATH, "master key");
  }

  private void assertDecryptionOfKey(
      final RsaEncryptionService rsaEncryptionService,
    ...
    assertThat(Hex.encodeHexString(actual))
        .withFailMessage("decrypted " + keyType)
  ...
  private RsaEncryptionService createRsaEncryptionServiceFromStoredKeys() {
    final KeyPair keyPair =
        new KeyPair(
            RsaEncryptionService.readPublicKeyFromFile(PUBLIC_KEY_PATH),
