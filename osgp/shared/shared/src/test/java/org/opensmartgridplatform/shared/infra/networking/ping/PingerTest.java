...
class PingerTest {
    ...
    private void whenPingCommandIsExecutedReceivingPackages() throws Exception {
        when(this.commandExecutor.execute(anyList(), any(Duration.class))).thenReturn(Arrays.asList(
    ...
    private void whenPingCommandIsExecutedWithoutReceivingPackages() throws Exception {
        when(this.commandExecutor.execute(anyList(), any(Duration.class))).thenReturn(Arrays.asList(
                ...
                "From 219.83.2.211 icmp_seq=1 Destination Net Unreachable", "", "--- 192.168.5.3 ping statistics ---",
    ...
    private void whenAnExceptionIsThrownProcessingInputFromThePingCommandBeforeTimeout() throws Exception {
        when(this.commandExecutor.execute(anyList(), any(Duration.class))).thenThrow(ExecutionException.class);
    }

    private void whenThePingCommandResultsAreNotObtainedBeforeTheTimeout() throws Exception {
        when(this.commandExecutor.execute(anyList(), any(Duration.class))).thenThrow(TimeoutException.class);
    }
}
