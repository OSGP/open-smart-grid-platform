...
class PingFlavorTest {
    ...
    void pingFlavorGivesCorrectCountFlag(final PingFlavor pingFlavor, final int count, final String countFlag) {
    ...
    void pingFlavorGivesEmptyFlagForNonPositiveCountFlag(final PingFlavor pingFlavor) {
        ...
        assertThat(pingFlavor.countFlag(nonPositive)).as("%s.countFlag(%d)", pingFlavor.name(), nonPositive).isEmpty();
    }

    @ParameterizedTest
    @CsvSource({ "LINUX, 58, -s 58", "LINUX, 0, -s 0", "MAC_OS, 27, -s 27", "MAC_OS, 0, -s 0", "WINDOWS, 64, -l 64",
            "WINDOWS, 0, -l 0" })
    void pingFlavorGivesCorrectSizeFlag(final PingFlavor pingFlavor, final int size, final String sizeFlag) {
    ...
    void pingFlavorGivesEmptyFlagForNegativeSizeFlag(final PingFlavor pingFlavor) {
        ...
        assertThat(pingFlavor.sizeFlag(negative)).as("%s.sizeFlag(%d)", pingFlavor.name(), negative).isEmpty();
    }

    @ParameterizedTest
    @CsvSource({ "LINUX, PT5S, -W 5", "MAC_OS, PT6S, -t 6", "WINDOWS, PT0.35S, -w 350" })
    void pingFlavorGivesCorrectTimeoutFlag(final PingFlavor pingFlavor, final Duration timeout,
            final String timeoutFlag) {
    ...
    void pingFlavorGivesEmptyFlagForNullTimeoutFlag(final PingFlavor pingFlavor) {
        assertThat(pingFlavor.timeoutFlag(null)).as("%s.timeoutFlag(null)", pingFlavor.name()).isEmpty();
    }
    ...
    void pingFlavorGivesEmptyFlagForZeroTimeoutFlag(final PingFlavor pingFlavor) {
        ...
        assertThat(pingFlavor.timeoutFlag(zeroTimeout)).as("%s.timeoutFlag(%s)", pingFlavor.name(), zeroTimeout)
                .isEmpty();
    }
    ...
    void pingFlavorGivesEmptyFlagForNonPositiveTimeoutFlag(final PingFlavor pingFlavor) {
        ...
        assertThat(pingFlavor.timeoutFlag(nonPositive)).as("%s.timeoutFlag(%s)", pingFlavor.name(), nonPositive)
                .isEmpty();
    }
    ...
    @ParameterizedTest
    @EnumSource(PingFlavor.class)
    void pingFlavorGivesEmptyFlagForDefaultLookupNamesForHostAddressesFlag(final PingFlavor pingFlavor) {
    ...
    @ParameterizedTest
    @CsvSource({ "LINUX, true, ''", "LINUX, false, -n", "MAC_OS, true, ''", "MAC_OS, false, -n", "WINDOWS, true, -a",
            "WINDOWS, false, ''" })
    void pingFlavorGivesCorrectLookupNamesForHostAddressesFlag(final PingFlavor pingFlavor, final Boolean lookupNames,
            final String lookupNamesForHostAddressesFlag) {

        assertThat(pingFlavor.lookupNamesForHostAddressesFlag(lookupNames)).isEqualTo(lookupNamesForHostAddressesFlag);
    }
}
