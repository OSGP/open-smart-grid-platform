...
public class CosemObjectDefinitionDto implements Serializable {
    ...
    public CosemObjectDefinitionDto(final int classId, final CosemObisCodeDto logicalName, final int attributeIndex,
            final int dataIndex) {
    ...
    private void checkFields(final int classId, final CosemObisCodeDto logicalName, final int attributeIndex,
            final int dataIndex) {
    ...
    private void checkAttributeIndex(final int attributeIndex) {
        if (attributeIndex < -128 || attributeIndex > 255) {
            throw new IllegalArgumentException("attributeIndex not in byte value range: " + attributeIndex);
        }
    }
    ...
    public String toDsmrString() {
        return String.format("{%d,%s,%d,%d}", this.classId, this.logicalName.toDsmrString(), this.attributeIndex,
                this.dataIndex);
    }
