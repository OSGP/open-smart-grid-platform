 ...
 * message} {@link Message#signature signatures}.
 *
 * <p>Using the defaults a {@value #DEFAULT_SIGNATURE_KEY_SIZE}-bits {@value
 * #DEFAULT_SIGNATURE_KEY_ALGORITHM} encrypted {@link #DEFAULT_SIGNATURE_ALGORITHM SHA-256} hash
 * over the message bytes (with its {@link Message#signature signature} set to {@code null}) is
 * created or verified.
 *
 * <p>Depending on whether a private signing key and/or a public verification key are present a
 * specific instance of the MessageSigner may or may not be able to {@link #sign(Message) sign a
 * message}, {@link #signature(Message) determine a signature for a message} or {@link
 * #verify(Message) verify the signature of a message}.<br>
 * Each MessageSigner {@link #canSignMessages() can sign messages} or {@link
 * #canVerifyMessageSignatures() can verify message signatures}.
 */
