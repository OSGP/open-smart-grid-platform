...
public class CosemTime implements Serializable, Comparable<CosemTime> {
  ...
  public CosemTime(final LocalTime time) {
    this(
        ...
        time.getMinute(),
        time.getSecond(),
        (int) (time.getLong(ChronoField.MILLI_OF_SECOND) / 10));
  }
  ...
  /**
   * Returns this {@link CosemTime} as {@link LocalTime} if {@code hour}, {@code minute} do not
   * contain wildcard values.
   *
   * @return this {@link CosemTime} as {@link LocalTime}, or {@code null} if not {@link
