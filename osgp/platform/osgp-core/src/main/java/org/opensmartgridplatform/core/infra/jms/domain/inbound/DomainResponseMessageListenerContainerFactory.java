...
public class DomainResponseMessageListenerContainerFactory implements InitializingBean, DisposableBean {
    ...
    private void init(final DomainInfo domainInfo) throws SSLException {
        final JmsConfigurationFactory jmsConfigurationFactory = new JmsConfigurationFactory(this.environment,
                this.defaultDomainJmsConfiguration, domainInfo.getIncomingResponsesPropertyPrefix());
        ...
        final DomainResponseMessageListener messageListener = new DomainResponseMessageListener(
                this.protocolResponseService, this.protocolInfos);
