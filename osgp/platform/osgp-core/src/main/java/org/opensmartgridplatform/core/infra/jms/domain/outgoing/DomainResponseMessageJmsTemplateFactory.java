...
public class DomainResponseMessageJmsTemplateFactory implements InitializingBean, DisposableBean {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DomainResponseMessageJmsTemplateFactory.class);
    ...
    public DomainResponseMessageJmsTemplateFactory(final Environment environment, final List<DomainInfo> domainInfos) {
    ...
    private void init(final DomainInfo domainInfo) throws SSLException {
        ...
        final JmsConfigurationFactory jmsConfigurationFactory = new JmsConfigurationFactory(this.environment,
                this.defaultDomainJmsConfiguration, domainInfo.getOutgoingResponsesPropertyPrefix());
