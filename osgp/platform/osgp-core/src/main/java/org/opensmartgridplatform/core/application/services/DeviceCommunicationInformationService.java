...
public class DeviceCommunicationInformationService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DeviceCommunicationInformationService.class);

    @Autowired
    private DeviceRepository deviceRepository;

    /**
     * Update the device record with information about the device connection.
     * Based on the response message, set the time stamps and counter for the
     * device.
     *
     * @param message
     *            The {@link ProtocolResponseMessage} containing the response
     *            message.
     */
    @Transactional
