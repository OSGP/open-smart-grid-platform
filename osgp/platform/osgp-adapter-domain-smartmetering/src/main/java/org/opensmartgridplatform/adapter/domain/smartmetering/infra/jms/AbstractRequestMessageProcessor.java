...
public abstract class AbstractRequestMessageProcessor {
    ...
    protected void handleError(final Exception e, final String correlationUid, final String organisationIdentification,
        ...
        if (e instanceof OsgpException) {
            ...
            osgpException = new TechnicalException(componentType,
                    String.format("An unknown error of type %s occurred.", e.getClass().getName()), e);
        }

        final ResponseMessage responseMessage = ResponseMessage.newResponseMessageBuilder().withCorrelationUid(
                correlationUid).withOrganisationIdentification(organisationIdentification).withDeviceIdentification(
                deviceIdentification).withResult(ResponseMessageResultType.NOT_OK).withOsgpException(
                osgpException).withMessagePriority(messagePriority).build();
