...
public class ResponseDataService {
  ...
  public void enqueue(final ResponseData responseData) {
    if (this.responseDataRepository.findByCorrelationUid(responseData.getCorrelationUid())
      ...
      LOGGER.warn(
          "Response data with correlation uid {} already exists. Skipping save action",
   ...
   *
   * <p>If correlationUid does not match type, null will be returned and the data will not be
   * removed from the database.
   *
   * @param correlationUid
   * @param expectedClassType type to check the data object against.
   * @return ReponseData Response data.
   * @throws FunctionalException when correlationUid is not found, or when the expected class type
   *     does not match.
   */
  public ResponseData dequeue(
  ...
  private void validateResponseData(
    ...
    if (!this.isValidResponseType(responseData, expectedClassType)) {
      ...
      LOGGER.warn(
          "Incorrect type of response data: {} for correlation UID: {}",
          warningResultClassType,
          responseData.getCorrelationUid());
  ...
  /**
   * ResponseData is valid when ResponseData message data type is equal to the expected type OR The
   * response message result type is NOT_OK, so the message data will be exception information.
   *
   * @param responseData response data.
   * @param expectedClassType expected class
   * @return is valid.
   */
  private boolean isValidResponseType(
      final ResponseData responseData, final Class<?> expectedClassType) {
