...
public class ApplicationContext extends AbstractConfig {
  ...
  public MessageSigner messageSigner(
      @Value("${low.voltage.kafka.message.signature.algorithm:SHA256withRSA}")
          final String signatureAlgorithm,
      @Value("${low.voltage.kafka.message.signature.provider:SunRsaSign}")
          final String signatureProvider,
      @Value("${low.voltage.kafka.message.signature.key.algorithm:RSA}")
          final String signatureKeyAlgorithm,
      @Value("${low.voltage.kafka.message.signature.key.size:2048}") final int signatureKeySize,
      @Value("${low.voltage.kafka.message.signature.key.private:#{null}}")
          final Resource signingKeyResource,
      @Value("${low.voltage.kafka.message.signature.key.public}")
    ...
    return MessageSigner.newBuilder()
        ...
        .verificationKey(
            this.readKeyFromPemResource(verificationKeyResource, "public verification key"))
        .build();
  }
