...
public class LowVoltageMeasurementToAnalogList implements StringArrayToAnalogList {
  ...
  /**
   * Returns the implementation class to be used to convert a specific number of measurement values.
   * <br>
   * The implementation class is an {@link Enum} that implements {@link
   * LowVoltageMeasurementDefinition}, with constants that belong with each value in the list the
   * {@link LowVoltageMeasurementDefinition#getIndex() index} of the {@link
   * LowVoltageMeasurementDefinition} links the definition to the position in the list of values.
   */
  ...
  public List<Analog> convertToAnalogList(final String[] values) {
    final Class<? extends LowVoltageMeasurementDefinition> implementation =
        implementation(values.length);
