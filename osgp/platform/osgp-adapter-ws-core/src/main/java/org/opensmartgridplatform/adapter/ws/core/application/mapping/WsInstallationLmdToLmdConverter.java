...
public class WsInstallationLmdToLmdConverter extends
        CustomConverter<org.opensmartgridplatform.adapter.ws.schema.core.deviceinstallation.LightMeasurementDevice, LightMeasurementDevice> {

    private static final Logger LOGGER = LoggerFactory.getLogger(WsInstallationLmdToLmdConverter.class);

    private final WritableDeviceModelRepository writableDeviceModelRepository;

    public WsInstallationLmdToLmdConverter(final WritableDeviceModelRepository writableDeviceModelRepository) {
        this.writableDeviceModelRepository = writableDeviceModelRepository;
    }

    @Override
    public LightMeasurementDevice convert(
            final org.opensmartgridplatform.adapter.ws.schema.core.deviceinstallation.LightMeasurementDevice source,
            final Type<? extends LightMeasurementDevice> destinationType, final MappingContext mappingContext) {
        ...
        final LightMeasurementDevice lmd = new LightMeasurementDevice(deviceIdentification, alias, containerAddress,
                gpsCoordinates, null);
