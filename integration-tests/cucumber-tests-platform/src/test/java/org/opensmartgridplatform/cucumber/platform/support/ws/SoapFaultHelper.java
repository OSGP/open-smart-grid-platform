...
public class SoapFaultHelper {
  ...
  public static Object getFaultDetailValuesByElement(
    ...
    final Node nodeForFirstFaultDetailEntry =
        getNodeForFirstFaultDetailEntry(soapFaultClientException.getSoapFault().getFaultDetail());
    ...
    LOGGER.debug(
        "Checking child nodes of SOAP fault detail entry: {{}}{}",
        nodeForFirstFaultDetailEntry.getNamespaceURI(),
        nodeForFirstFaultDetailEntry.getLocalName());
  ...
  private static Node getNodeForFirstFaultDetailEntry(final SoapFaultDetail faultDetail) {
    ...
    if ((faultDetail == null)
        || (faultDetail.getDetailEntries() == null)
  ...
  private static Map<FaultDetailElement, String> getFaultDetailValuesByElement(
      final NodeList nodeList) {

    final Map<FaultDetailElement, String> faultDetailValuesByElement =
        new EnumMap<>(FaultDetailElement.class);
  ...
  private static void addFaultDetailElement(
      final Map<FaultDetailElement, String> faultDetailValuesByElement, final Node node) {

    final FaultDetailElement faultDetailElement =
        FaultDetailElement.forLocalName(node.getLocalName());

    if (faultDetailElement == null) {
      LOGGER.info(
          "Ignoring unexpected child node: {{}}{}", node.getNamespaceURI(), node.getLocalName());
      return;
    }
