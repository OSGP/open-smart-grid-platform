...
public abstract class AbstractFindEventsReads {
  ...
  @Autowired
  private SmartMeteringManagementRequestClient<FindEventsAsyncResponse, FindEventsRequest>
      smartMeteringManagementRequestClient;

  @Autowired
  private SmartMeteringManagementResponseClient<FindEventsResponse, FindEventsAsyncRequest>
  ...
  public void receivingAFindEventsRequest(final Map<String, String> requestData) throws Throwable {
    ...
    settings.put(
        ...
        requestData.get(PlatformSmartmeteringKeys.KEY_END_DATE));
    settings.put(
    ...
    final FindEventsAsyncResponse findEventsAsyncResponse =
        this.smartMeteringManagementRequestClient.doRequest(findEventsRequest);
  ...
  public void eventsShouldBeReturned(final Map<String, String> settings) throws Throwable {
    final FindEventsAsyncRequest findEventsAsyncRequest =
        FindEventsRequestFactory.fromScenarioContext();
    final FindEventsResponse findEventsResponse =
        this.smartMeteringManagementResponseClient.getResponse(findEventsAsyncRequest);
    ...
    assertThat(findEventsResponse.getEvents().size())
        .as("Number of events should match")
  ...
  public void eventsShouldBeReturned(final int numberOfEvents, final Map<String, String> settings)
      throws Throwable {
    this.eventsShouldBeReturned(
        SettingsHelper.addDefault(
            settings, EXPECTED_NUMBER_OF_EVENTS, String.valueOf(numberOfEvents)));
  }
