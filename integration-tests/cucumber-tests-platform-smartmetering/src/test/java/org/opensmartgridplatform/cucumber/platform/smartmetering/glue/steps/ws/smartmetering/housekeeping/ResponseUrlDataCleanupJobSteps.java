...
import org.opensmartgridplatform.cucumber.platform.smartmetering.glue.steps.database.ws.SmartMeteringResponseUrlDataSteps;
...
public class ResponseUrlDataCleanupJobSteps {
  ...
  @Autowired private SmartMeteringResponseUrlDataSteps responseUrlDataSteps;
  ...
  @Then(
      "^the cleanup job should not have removed the response url data with correlation uid \"(.*)\"$")
  ...
  private void waitForResponseUrlDataToBeRemoved(
      final String correlationUid, final long delay, final int retries) {
    try {
      RetryableAssert.assertWithRetries(
          () -> this.responseUrlDataSteps.theResponseUrlDataRecordShouldBeDeleted(correlationUid),
          retries,
          delay,
          TIME_UNIT_RESPONSE_CLEANUP_DELAY);
    } catch (final AssertionError e) {
      Assertions.fail(
          "Cleanup job should have removed response url data with correlation uid "
              + correlationUid
              + " within "
              + RetryableAssert.describeMaxDuration(
                  retries, delay, TIME_UNIT_RESPONSE_CLEANUP_DELAY));
    }
  }

  private void waitToMakeSureResponseUrlDataIsNotRemoved(
      final String correlationUid, final long delay, final int retries) {

    try {
      RetryableAssert.assertDelayedWithRetries(
          () ->
              this.responseUrlDataSteps.theResponseUrlDataRecordShouldNotBeDeleted(correlationUid),
          0,
          retries * delay,
          TIME_UNIT_RESPONSE_CLEANUP_DELAY);
    } catch (final AssertionError e) {
      Assertions.fail(
          "Cleanup job should not have removed response url data with correlation uid "
              + correlationUid
              + ".");
    }
  }
