...
public class CaptureObjectsFactory {
  ...
  public static CaptureObjectDefinitions fromParameterMap(
    ...
    final List<CaptureObjectDefinition> captureObjects =
        captureObjectDefinitions.getCaptureObject();

    final int numberOfCaptureObjects =
        getInteger(requestParameters, PlatformSmartmeteringKeys.NUMBER_OF_CAPTURE_OBJECTS, 0);
    for (int i = 1; i <= numberOfCaptureObjects; i++) {
      ...
      captureObjectDefinition.setClassId(
          SettingsHelper.getIntegerValue(
              requestParameters, PlatformSmartmeteringKeys.CAPTURE_OBJECT_CLASS_ID, i));
      ...
      captureObjectDefinition.setAttributeIndex(
          SettingsHelper.getByteValue(
              requestParameters, PlatformSmartmeteringKeys.CAPTURE_OBJECT_ATTRIBUTE_INDEX, i));
      captureObjectDefinition.setDataIndex(
          SettingsHelper.getIntegerValue(
              requestParameters, PlatformSmartmeteringKeys.CAPTURE_OBJECT_DATA_INDEX, i));
  ...
  private static ObisCodeValues logicalNameFromParemeterMap(
    ...
    final String logicalName =
        SettingsHelper.getStringValue(
            requestParameters, PlatformSmartmeteringKeys.CAPTURE_OBJECT_LOGICAL_NAME, i);
