...
Feature: SmartMetering Installation - Couple M-Bus Device
  ...
  Scenario: Couple G-meter to an E-meter, with meter already connected
    Given a dlms device
      | DeviceIdentification | TEST1024000000001 |
      | DeviceType           | SMART_METER_E     |
    And device simulation of "TEST1024000000001" with M-Bus client version 0 values for channel 1
  ...
  Scenario: Couple G-meter to an E-meter, with meter already connected to other device
    Given a dlms device
      | DeviceIdentification | TEST1024000000001 |
      | DeviceType           | SMART_METER_E     |
    And a dlms device
    ...
    And a SOAP fault should have been returned
      | Code    |                               216 |
      | Message | GIVEN_MBUS_DEVICE_ALREADY_COUPLED |
    And the M-Bus device "TESTG101205673117" is coupled to device "TEST1024000000002" on M-Bus channel "1" with PrimaryAddress "3"
    And the mbus device "TESTG101205673117" is not coupled to the device "TEST1024000000001"

  Scenario: Couple G-meter to an E-meter with force, with meter already connected
    Given a dlms device
      | DeviceIdentification | TEST1024000000001 |
      | DeviceType           | SMART_METER_E     |
    And device simulation of "TEST1024000000001" with M-Bus client version 0 values for channel 1
  ...
  Scenario: Couple G-meter to an E-meter with force, with meter already connected to other device
    Given a dlms device
      | DeviceIdentification | TEST1024000000001 |
      | DeviceType           | SMART_METER_E     |
    And a dlms device
