...
Feature: SmartMetering Bundle - FindEvents
  ...
  Scenario Outline: Retrieve event codes for meter with protocol <protocol> <version>
    ...
    And the bundle response should contain a find events response with <thd> events
    Examples:
      | deviceIdentification | protocol | version | std | fraud | comm | mbus | powq | aux | powqe | thd |
  ...
  Scenario Outline: Retrieve event codes for meter with protocol <protocol> <version> throws an Exception
    ...
    And the bundle response should <thd> a FaultResponse with message containing
      | Message | No address found for POWER_QUALITY_THD_EVENT_LOG in protocol <protocol> <version>      |

    Examples:
      | deviceIdentification | protocol | version | std    | fraud  | comm   | mbus   | powq   | aux    | powqe  | thd    |
      | TEST1026000000001    | DSMR     |     2.2 | not be | not be | be     | not be | be     | be     | be     | be     |
      | TEST1024000000001    | DSMR     |   4.2.2 | not be | not be | not be | not be | be     | be     | be     | be     |
