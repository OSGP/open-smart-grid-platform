...
Feature: SmartMetering Configuration - Replace Keys
  ...
  Scenario: Replace keys on a device that has previously created keys with status NEW
    Given a dlms device
      | DeviceIdentification | TEST1024000000480 |
      | DeviceType           | SMART_METER_E     |
    And new keys are registered in the secret management database with creationDate 1440 minutes ago
      | DeviceIdentification | TEST1024000000480 |
    ...
    And new keys are registered in the secret management database with creationDate 2880 minutes ago
      | DeviceIdentification | TEST1024000000480 |
      | Authentication_key   | 867424ac75b6d53c89276d304608321f0a1f6e401f453f84adf3477c7ee1623c |
      | Encryption_key       | c19fe80a22a0f6c5cdaad0826c4d204f23694ded08d811b66e9b845d9f2157d2 |
    When the replace keys request is received
    ...
    And the new keys are stored in the secret management database encrypted_secret table
    And the stored keys are not equal to the received keys
    And the encrypted_secret table in the secret management database should contain "ACTIVE" keys for device "TEST1024000000480"
      | E_METER_MASTER_KEY         |                 1 |
      | E_METER_AUTHENTICATION_KEY |                 1 |
      | E_METER_ENCRYPTION_KEY_UNICAST     |                 1 |
    And the encrypted_secret table in the secret management database should contain "EXPIRED" keys for device "TEST1024000000480"
      | E_METER_MASTER_KEY         |                 0 |
      | E_METER_AUTHENTICATION_KEY |                 2 |
      | E_METER_ENCRYPTION_KEY_UNICAST     |                 2 |
    And the encrypted_secret table in the secret management database should contain "NEW" keys for device "TEST1024000000480"
      | E_METER_MASTER_KEY         |                 0 |
      | E_METER_AUTHENTICATION_KEY |                 0 |
      | E_METER_ENCRYPTION_KEY_UNICAST     |                 0 |
