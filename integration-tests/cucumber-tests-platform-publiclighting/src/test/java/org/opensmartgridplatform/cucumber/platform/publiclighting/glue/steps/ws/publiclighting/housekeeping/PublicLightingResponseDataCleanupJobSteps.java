...
public class PublicLightingResponseDataCleanupJobSteps {
    ...
    @Autowired
    private PublicLightingResponseDataSteps responseDataSteps;
    ...
    @Then("^the public lighting cleanup job should have removed the response data with correlation uid \"(.*)\"$")
    @Transactional("txMgrWsPublicLighting")
    public void thePublicLightingCleanupJobShouldHaveRemovedTheResponseData(final String correlationUid) {

        this.waitForResponseDataToBeRemoved(correlationUid, this.delay, this.retries,
                this.publicLightingResponseDataRepository);
    }
    ...
    @Then("^the public lighting cleanup job should not have removed the response data with correlation uid \"(.*)\"$")
    @Transactional("txMgrWsPublicLighting")
    public void thePublicLightingCleanupJobShouldNotHaveRemovedTheResponseData(final String correlationUid) {
    ...
    private void waitForResponseDataToBeRemoved(final String correlationUid, final long delay, final int retries,
        ...
        try {
            RetryableAssert.assertWithRetries(() -> this.responseDataSteps
    ...
    private void waitToMakeSureResponseDataIsNotRemoved(final String correlationUid, final long delay,
            final int retries, final ResponseDataRepository responseDataRepository) {

        try {
            RetryableAssert.assertDelayedWithRetries(() -> this.responseDataSteps
