...
public class FindScheduledTasksSteps {
    ...
    public void givenScheduleTasks(final DataTable tasks) {
        ...
        scheduledTasks.stream().forEach(m -> this.scheduledTaskSteps.givenAScheduledTask(m.get(KEY_MESSAGE_TYPE), m));
    }
    ...
    public void thenTheFindScheduledTasksResponseShouldContain(final DataTable tasks) {
        final FindScheduledTasksResponse response = (FindScheduledTasksResponse) ScenarioContext.current()
                .get(RESPONSE);
    ...
    private static final boolean isEqual(final XMLGregorianCalendar actual, final String expected) {
        final DateTime expectedDateTime = DateTimeHelper.shiftSystemZoneToUtc(DateTimeHelper.getDateTime(expected));
