...
public class EventMessageDataContainerConverter extends
        CustomConverter<EventMessagesResponse, org.opensmartgridplatform.adapter.ws.schema.smartmetering.management.FindEventsResponseData> {

    @Override
    public FindEventsResponseData convert(final EventMessagesResponse source,
            final Type<? extends FindEventsResponseData> destinationType, final MappingContext context) {
        if (source == null) {
            return null;
        }
        ...
        for (final Event event : source.getEvents()) {
            final org.opensmartgridplatform.adapter.ws.schema.smartmetering.management.Event eventResponse = this.mapperFacade
                    .map(event, org.opensmartgridplatform.adapter.ws.schema.smartmetering.management.Event.class);
