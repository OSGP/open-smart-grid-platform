...
public class CorrelationUidTargetedNotificationService<T> extends DefaultNotificationService<T> {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(CorrelationUidTargetedNotificationService.class);
    ...
    @Override
    public String getCustomTargetUri(final NotificationWebServiceLookupKey endpointLookupKey,
            final GenericNotification notification) {

        final String customUri = this.responseUrlService.findResponseUrl(notification.getCorrelationUid());
        if (customUri != null) {
