...
do
  if [[ ! $value =~ "b:" ]]; then
    ...
    if [ -z "$(git ls-remote --exit-code --heads origin "${release_branch}")" ]; then
      echo "::error:: Source branch doesn't exist ${release_branch}"
      exit 1
    fi
    git checkout "${release_branch}"

    echo "::notice:: Checking POM version $expected_version"
    if [ -f pom.xml ]; then
      if grep -1 "<version>${expected_version}</version>" pom.xml; then
        echo "::notice:: Expected release found ${expected_version}"
      else
        echo "::warning:: Expected release not found $expected_version, found: $(grep '<version>' pom.xml | head -1)"
        warning=1
      fi
    else
