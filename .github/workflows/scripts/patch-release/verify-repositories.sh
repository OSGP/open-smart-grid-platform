...
error=0
old_version=$OLD_RELEASE_VERSION
...
do
  if [[ ! $value =~ "b:" ]]; then
    ...
    if [ -z "$(git ls-remote --exit-code --heads origin "${old_release_branch}")" ]; then
      echo "::error:: Source branch doesn't exist ${old_release_branch}"
      exit 1
    fi
    git checkout "${old_release_branch}"

    echo "::notice title=Checking branch not exists:$new_release_branch for repo $value"
    if [ -n "$(git ls-remote --exit-code --heads origin "${new_release_branch}")" ]; then
      echo "::error:: Release branch exists ${new_release_branch} for repo $value"
      error=1
    else
