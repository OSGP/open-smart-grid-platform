...
public class Iec61850RtuWriteProfileCommandFactory implements RtuWriteCommandFactory<ProfileDto> {
    ...
    private Map<String, RtuWriteCommand<ProfileDto>> rtuCommandMap = new HashMap<>();

    private Iec61850RtuWriteProfileCommandFactory() {
        for (int i = ID_START; i <= ID_END; i++) {

            this.rtuCommandMap.put(this.createMapKey(DataAttribute.SCHEDULE_ABS_TIME, i),
                    new Iec61850ScheduleAbsTimeCommand(i));
        }
    }

    public static Iec61850RtuWriteProfileCommandFactory getInstance() {
        if (instance == null) {
    ...
    public RtuWriteCommand<ProfileDto> getCommand(final String node) {
        ...
        final RtuWriteCommand<ProfileDto> command = this.rtuCommandMap.get(node);
