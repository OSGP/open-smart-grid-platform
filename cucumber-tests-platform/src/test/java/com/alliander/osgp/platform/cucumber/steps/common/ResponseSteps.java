...
public class ResponseSteps {
	...
	public static void VerifyFaultResponse(final RunXpathResult runXpathResult, final String response, final Map<String, String> expectedResult) throws XPathExpressionException, ParserConfigurationException, SAXException, IOException {
		...
		if (expectedResult.containsKey("FaultCode")) {
			Assert.assertTrue(runXpathResult.assertXpath(response, "/Envelope/Body/Fault/faultcode", expectedResult.get("FaultCode")));
		}
		...
		if (expectedResult.containsKey("FaultType")) {
			if (expectedResult.get("FaultType").equals("ValidationError")) {
				...
				for (int i = 1; i <= validationErrors.length; i++) {
					...
					//Assert.assertTrue(runXpathResult.assertXpath(response, "(/Envelope/Body/Fault/ValidationError)["+i+"]", validationErrors[i]));
				}
			} else {
				// FunctionalError or TechnicalError
				if (expectedResult.containsKey("Code")) {
					Assert.assertTrue(runXpathResult.assertXpath(response, "/Envelope/Body/Fault/detail/" + expectedResult.get("FaultType") + "/Code", expectedResult.get("Code")));
				}
				if (expectedResult.containsKey("Message")) {
					Assert.assertTrue(runXpathResult.assertXpath(response, "/Envelope/Body/Fault/detail/" + expectedResult.get("FaultType") + "/Message", expectedResult.get("Message")));
				}
				if (expectedResult.containsKey("Component")) {
					Assert.assertTrue(runXpathResult.assertXpath(response, "/Envelope/Body/Fault/detail/" + expectedResult.get("FaultType") + "/Component", expectedResult.get("Component")));
				}
				if (expectedResult.containsKey("InnerException")) {
					Assert.assertTrue(runXpathResult.assertXpath(response, "/Envelope/Body/Fault/detail/" + expectedResult.get("FaultType") + "/InnerException", expectedResult.get("InnerException")));
				}
				if (expectedResult.containsKey("InnerMessage")) {
					Assert.assertTrue(runXpathResult.assertXpath(response, "/Envelope/Body/Fault/detail/" + expectedResult.get("FaultType") + "/InnerMessage", expectedResult.get("InnerMessage")));
				}
			}
		}
	}
