...
public class GetDataRequestBuilder {
    ...
    private static void addSystemFilters(final Map<String, String> requestParameters,
        ...
        if (!requestParameters.containsKey(PlatformKeys.KEY_NUMBER_OF_SYSTEMS)) {
            throw new AssertionError("The Step DataTable must contain the number of system filters for key \""
                    + PlatformKeys.KEY_NUMBER_OF_SYSTEMS + "\" when creating a get data request.");
        }
        final int numberOfSystems = Integer.parseInt(requestParameters.get(PlatformKeys.KEY_NUMBER_OF_SYSTEMS));
    ...
    private static void addSystemFilter(final Map<String, String> requestParameters,
        ...
        systemFilter.setId(Integer.parseInt(requestParameters.get(PlatformKeys.KEY_SYSTEM_ID.concat(indexPostfix))));
    ...
    private static void addMeasurementFilters(final Map<String, String> requestParameters,
        ...
        if (!requestParameters.containsKey(PlatformKeys.KEY_NUMBER_OF_MEASUREMENTS.concat(indexPostfix))) {
            return;
        }
        final int numberOfMeasurements = Integer
                .parseInt(requestParameters.get(PlatformKeys.KEY_NUMBER_OF_MEASUREMENTS.concat(indexPostfix)));
    ...
    private static void addMeasurementFilter(final Map<String, String> requestParameters,
        ...
        if (requestParameters.containsKey(PlatformKeys.KEY_MEASUREMENT_FILTER_ID.concat(indexPostfix))) {
            measurementFilter.setId(
                    Integer.parseInt(requestParameters.get(PlatformKeys.KEY_MEASUREMENT_FILTER_ID.concat(indexPostfix))));
        }
        measurementFilter.setNode(requestParameters.get(PlatformKeys.KEY_MEASUREMENT_FILTER_NODE.concat(indexPostfix)));
